...
class KlinesFromDatabase:
    ...
    def one_minute_klines_dataframe(self, asset_symbol, number_of_one_minute_entries):
    ...
    def get_klines(self, asset_symbol, candle_interval, number_of_candles):
        
        klines = pd.DataFrame(columns = ['open_time', 'open', 'high', 'low', 'close', 'volume'])

        if (candle_interval == '1m'): 

            klines = self.one_minute_klines_dataframe(asset_symbol, number_of_candles)
            ...
            klines_in = self.one_minute_klines_dataframe(asset_symbol, number_of_entries)
